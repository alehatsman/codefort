...
export async function mockApi(page: Page, seed: Partial<State> = {}): Promise<State> {
  ...
  await page.route(/\/api\/repos\/[^/]+\/[^/]+$/, (route) => {
    ...
    if (req.method() === "PATCH") {
      ...
      if (Array.isArray(body.protected_refs)) repo.protected_refs = body.protected_refs
    }
  ...
  // Force-stop a run (agent or CI) from any non-terminal state -> canceled.
  ...
  await page.route(/\/api\/repos\/[^/]+\/[^/]+\/pulls(\?.*)?$/, (route) => {
    ...
    // matching the server's parsePRStates, then by ?q= keyword over title/body.
    ...
    let out = wants.length ? state.pulls.filter((p) => wants.includes(p.state)) : state.pulls
    if (q) {
      out = out.filter(
