...
export default function CommitPage() {
  ...
  function copySha() {
    navigator.clipboard?.writeText(detail?.commit.sha ?? sha).then(
      ...
      () => {
        // Clipboard write can be denied (permissions) — nothing to do about it here.
      }
    )
  }
      ...
      {commitQ.isLoading ? (
        ...
        <>
          <CommitHeader
            ...
            onCopySha={copySha}
          />
          <DiffSummary detail={detail} mode={mode} setMode={setMode} />
        </>
      )}
    </div>
  )
...
type CommitDetail = NonNullable<ReturnType<typeof useCommit>["data"]>
      ...
      <div className="commit-detail__meta muted small">
        ...
        <span className="commit-detail__sha-group">
          <Tooltip label={copied ? "Copied!" : "Copy full SHA"}>
            <button type="button" className="commit-detail__sha" onClick={onCopySha}>
        ...
        {detail.parents.length > 0 && (
          <span className="commit-detail__parents">
            ...
            {detail.parents.map((p) => (
              <Link key={p} to={`/${owner}/${repo}/commit/${p}`} className="commit-detail__parent">
                {p.slice(0, 7)}
              </Link>
            ))}
          </span>
  ...
  return (
    <>
      ...
      {detail.files.length === 0 ? (
        ...
        detail.files.map((f) => <DiffView key={f.new_path || f.old_path} file={f} mode={mode} />)
      )}
    </>
  )
}
