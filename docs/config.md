...
| `MOONGIT_CI_POLL_INTERVAL` | `5s` | How often an idle runner polls for a queued run. Zero or negative falls back to `5s`; anything under `100ms` is floored there (with a warning), since sub-100ms polling only spins SQLite. |
