...
| `CODEFORT_DB_PATH` | `$CODEFORT_DATA_DIR/codefort.db` | SQLite database file. Split out only if you want the DB on different storage than the repos. Renamed from `moongit.db`: an existing deployment must move the file (and its `-wal`/`-shm` sidecars) with the service stopped. `codefortd` refuses to start if `codefort.db` is missing while `moongit.db` sits beside it, rather than creating an empty database next to the real one. |
