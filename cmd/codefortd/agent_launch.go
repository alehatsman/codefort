...
// (`--resume`) agree without persisting anything, and so it survives a codefortd
// restart. claude requires `--session-id` to be UUID-shaped, hence the version
// and variant nibbles.
func agentSessionID(runID int64) string {
	h := sha1.Sum([]byte(fmt.Sprintf("codefort-agent-run-%d", runID))) //nolint:gosec // G401: UUIDv5 is defined to use SHA-1; not a security hash.
	var b [16]byte
	copy(b[:], h[:16])
...
// agent MCP servers (cf), restricting claude to
...
// composeAgentSystemPrompt builds the codefort-authored system prompt that
...
func composeAgentSystemPrompt(owner, repo string, issue api.Issue) string {
	...
	fmt.Fprintf(&b, "You are an autonomous coding agent working on the codefort repository %s/%s.\n", owner, repo)
	...
	b.WriteString("by codefort. codefort snapshots your file changes into a branch and posts the summary ")
...
func composeVerifySystemPrompt(owner, repo string) string {
	var b strings.Builder
	fmt.Fprintf(&b, "You are a spec-verification agent for the codefort repository %s/%s.\n", owner, repo)
