...
// runPR dispatches `cf pr <subcommand>` — a thin client over the PR data
// plane and merge endpoint, mirroring `cf issue` / `cf review`.
func runPR(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: cf pr <create|list|show|merge|close|reopen>")
	}
	switch args[0] {
	case "create":
...
func runPRCreate(args []string) error {
	...
	if *base == "" || *head == "" || strings.TrimSpace(*title) == "" {
		return errors.New("usage: cf pr create --base <ref> --head <ref> --title <t> [--body <b>]")
	}
...
func runPRShow(args []string) error {
	if len(args) != 1 {
		return errors.New("usage: cf pr show <number>")
	}
...
func runPRMerge(args []string) error {
	if len(args) < 1 {
		return errors.New("usage: cf pr merge <number> [--ff-only | --rebase]")
	}
	num, err := strconv.Atoi(args[0])
	if err != nil || num <= 0 {
		return fmt.Errorf("invalid pull request number: %s", args[0])
...
func runPRSetState(args []string, state api.PRState) error {
	...
	if len(args) < 1 {
		return fmt.Errorf("usage: cf pr %s <number>", verb)
	}
	num, err := strconv.Atoi(args[0])
	if err != nil || num <= 0 {
		return fmt.Errorf("invalid pull request number: %s", args[0])
