...
Self-hosted git host + issue tracker (Go daemon `moongitd` + Vite web UI).
Serves `/api/*`, git smart-HTTP, and the SPA on one port. `mgit` is the
client. This repo is the coordination backend itself — dogfood it.
