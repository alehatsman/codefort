# codefort CI base image

When CI runs with container isolation (the default — `CODEFORT_CI_ISOLATION=docker`),
codefortd executes each job inside a throwaway container as one streamed
...
directory builds the default image, `codefort-ci:latest`.
...
1. **Produce a static `mooncake` binary** into `ci/mooncake`. A static build
   ...
   CGO_ENABLED=0 go build -o /path/to/codefort/ci/mooncake ./cmd
   ```
...
2. **Produce a `provision` binary** into `ci/provision`:
   ...
   cargo build --release
   cp target/release/provision /path/to/codefort/ci/provision
   ```
...
| `CODEFORT_CI_DEFAULT_IMAGE` | `codefort-ci:latest` | image a job uses when its `codefort.yml` doesn't set `image:`. |
...
- **Manual trigger** for an arbitrary ref (branch, tag, or commit SHA):
  `cf ci run <ref>`, or `POST /api/repos/{owner}/{repo}/runs` with body
...
`ci/Dockerfile.dev` builds one such image, `codefort-ci-dev:latest` (base + Go +
Node), which codefort's own `codefort.yml` runs on:
...
(`go: not found`), `cf ci validate` warns up front when a job runs a known
toolchain (go, npm, …) but pins no `image:`.

## Runtime notes

- Containers are named `codefort-ci-<jobID>-<job>` and started detached
  (`sleep infinity`); each is removed (`docker rm -f`) when its job finishes.
- A container runs as the codefortd `uid:gid` (`docker run --user`) with the
